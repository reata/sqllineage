...
def test_insert_into_partitions():
    helper("INSERT INTO TABLE tab1 PARTITION (par1=1) SELECT * FROM tab2", {"tab2"}, {"tab1"})
...
def test_with_insert():
    helper("WITH tab1 AS (SELECT * FROM tab2) INSERT INTO tab3 SELECT * FROM tab1", {"tab2"}, {"tab3"})


def test_with_insert_overwrite():
    helper("WITH tab1 AS (SELECT * FROM tab2) INSERT OVERWRITE tab3 SELECT * FROM tab1", {"tab2"}, {"tab3"})
...
def test_with_insert_overwrite_plus_keyword_table():
    helper("WITH tab1 AS (SELECT * FROM tab2) INSERT OVERWRITE TABLE tab3 SELECT * FROM tab1", {"tab2"}, {"tab3"})
